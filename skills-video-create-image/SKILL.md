...
```bash
curl -N -X POST "https://open.skills.video/api/v1/generation/sse/google/nano-banana-pro" \
  -H "Authorization: Bearer $SKILLS_VIDEO_API_KEY" \
  -H "Accept: text/event-stream" \
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "Minimal product photo of a matte black coffee grinder on white background"
  }'
```
